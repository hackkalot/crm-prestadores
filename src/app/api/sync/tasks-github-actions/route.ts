...
/**
 * POST /api/sync/tasks-github-actions
 *
 * Triggers the GitHub Actions workflow to sync tasks data.
 * This endpoint works in production (Vercel) by calling GitHub's API.
 *
 * No body required - tasks sync exports all data (no date filters)
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (logError) {
      console.error('Failed to create tasks sync log:', logError)
    }

    // Trigger GitHub Actions workflow via repository_dispatch
    console.log(`Triggering GitHub Actions for tasks sync`)
