...
const RECURRENCES_URL = 'https://fidelidadep10.outsystemsenterprise.com/FixoBackoffice/ServiceRequestRecurrencies';
...
const OUTPUT_PATH = path.join(DATA_PATH, 'recurrences-outputs');
const LOG_FILE = path.join(DATA_PATH, `recurrences-scrapper_${new Date().toISOString().split('T')[0]}.log`);

// TIPOS
export interface RecurrencesScrapperOptions {
    outputPath?: string
    headless?: boolean
}

export interface RecurrencesScrapperResult {
    success: boolean
    filePath?: string
    error?: string
    recordCount?: number
...
/**
 * Funcao principal exportavel para scrapping de recorrencias do backoffice
 */
export async function runRecurrencesScrapper(options: RecurrencesScrapperOptions = {}): Promise<RecurrencesScrapperResult> {
    const {
    ...
    log('===============================================');
    log('SCRAPPER DE RECORRENCIAS DO BACKOFFICE');
    log('===============================================');
    ...
    try {
        ...
        // Navegar para pagina de recorrencias
        log('Navegando para pagina de recorrencias...');
        await page.goto(RECURRENCES_URL, { waitUntil: 'networkidle2', timeout: 60000 });
