-- Fix: services is text[] not jsonb, use correct array operator
...
-- Create function with correct text array matching
...
BEGIN
  ...
  LEFT JOIN providers p
    ON p.status = 'ativo'
    -- Exact match: services is text[], use array contains operator
...
-- Comments
COMMENT ON FUNCTION get_provider_coverage_by_service IS 'Retorna cobertura de prestadores por serviço e concelho. Usa correspondência exata de serviços (text[]) e filtra por distrito.';
