-- Fix: Filter providers by district coverage
-- Bug: Previous version counted all providers nationwide that offer a service,
-- instead of only those that cover the specific district of the request
...
-- Create corrected function that filters providers by district
...
BEGIN
  ...
  SELECT
    ...
    -- CORRIGIDO: Agora filtra também pelo distrito do pedido
    COUNT(DISTINCT CASE
...
-- Comments
COMMENT ON FUNCTION get_provider_coverage_by_service IS 'Retorna cobertura de prestadores por serviço e concelho, com período configurável em meses. Filtra prestadores pelo distrito que cobrem.';
