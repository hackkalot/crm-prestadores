...
/**
 * Generate HTML content for PDF pricing proposal (simplified structure from pricing-actions)
 */
export function generateProposalPDFHTML(data: {
  provider: {
    id: string
    name: string
    nif: string | null
  ...
  for (const { category, services } of data.pricingTable) {
    for (const service of services) {
      totalServices++
      totalPriceWithoutVat += service.provider_price
      const vatAmount = service.provider_price * (category.vat_rate / 100)
      const totalWithVat = service.provider_price + vatAmount

      rows.push(`
        <tr>
          <td style="border-bottom: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 13px;">
            ${service.name}
            ${service.variant_name ? `<br/><span style="color: #6b7280; font-size: 12px;">${service.variant_name}</span>` : ''}
          </td>
          <td style="border-bottom: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 13px;">
            ${service.unit || '-'}
          </td>
          <td style="border-bottom: 1px solid #e5e7eb; padding: 12px; text-align: right; font-size: 13px;">
            €${service.provider_price.toFixed(2)}
          </td>
          <td style="border-bottom: 1px solid #e5e7eb; padding: 12px; text-align: center; font-size: 13px;">
            ${category.vat_rate}%
          </td>
  ...
  for (const { category, services } of data.pricingTable) {
    ...
    for (const service of services) {
      ...
      const cluster = clusterMap.get(clusterName)!
      cluster.count++
      cluster.total += service.provider_price
    }
  }
  ...
  const clusterSummary = Array.from(clusterMap.entries())
    .map(
      ...
      <tr style="background-color: #f3f4f6;">
        ...
        <td style="padding: 10px 12px; text-align: center; font-weight: 600; font-size: 13px;">
          ${clusterData.count} serviço${clusterData.count !== 1 ? 's' : ''}
        </td>
        <td style="padding: 10px 12px; text-align: right; font-weight: 600; font-size: 13px;">
          €${clusterData.total.toFixed(2)}
        </td>
      </tr>
    `
    )
  ...
  for (const { category, services } of data.pricingTable) {
    for (const service of services) {
      totalVat += service.provider_price * (category.vat_rate / 100)
    }
  }
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Proposta de Preços - ${data.provider.name}</title>
      <style>
        * {
          margin: 0;
          padding: 0;
      ...
      <div class="container">
        ...
        <div class="provider-info">
          ...
          <div class="provider-details">
            <div class="detail-row">
              <span class="detail-label">Nome</span>
              <span class="detail-value">${data.provider.name}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">Email</span>
              <span class="detail-value">${data.provider.email}</span>
            </div>
            ${data.provider.nif ? `
              <div class="detail-row">
                <span class="detail-label">NIF</span>
                <span class="detail-value">${data.provider.nif}</span>
              </div>
            ` : ''}
          </div>
        </div>
