...
/**
 * POST /api/sync/billing-github-actions
 *
 * Triggers the GitHub Actions workflow to sync billing processes data.
 * This endpoint works in production (Vercel) by calling GitHub's API.
 *
 * No body required - billing sync exports all data (no date filters)
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (logError) {
      console.error('Failed to create billing sync log:', logError)
    }

    // Trigger GitHub Actions workflow via repository_dispatch
    console.log(`🚀 Triggering GitHub Actions for billing sync`)
